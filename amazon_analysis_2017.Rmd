...
bayes_mod <- brm(as.numeric(Correct) ~ TrialType * scale(Age) 
                 ...
                 iter = 5000, 
                 save_all_pars = TRUE)
save(bayes_mod, file = "bayes_mod.Rds")
```

```{r}
load(file = "bayes_mod.Rds")
kable(fixef(bayes_mod), digits = 2)
```

```{r, eval = FALSE}
bayes_mod_noagestim <- brm(as.numeric(Correct) ~ TrialType * scale(Age) 
                 + (TrialType | Subid) 
                 + (TrialType | Stimulus), 
                 family = "bernoulli", 
                 data = filter(d, age_grp != "exclude"),
                 ...
                 iter = 5000, 
                 save_all_pars = TRUE)
save(bayes_mod_noagestim, file = "bayes_mod_noagestim.Rds")
```

```{r}
load("bayes_mod_noagestim.Rds")
kable(fixef(bayes_mod_noagestim), digits = 2)
```

Bayes factor analysis suggests that the age by stimulus slopes are very much preferred. 
```{r}
# h0.bridge <- bridge_sampler(bayes_mod_noagestim, silent = TRUE, maxiter = 20000)
# h1.bridge <- bridge_sampler(bayes_mod, silent = TRUE, maxiter = 20000)

bf <- bayes_factor(bayes_mod, bayes_mod_noagestim, 
                   maxiter = 20000, verbose=FALSE, log=TRUE)

bf
```


Investigate age effects in item differences. 
